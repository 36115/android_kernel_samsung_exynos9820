...
Options:
    ...
    -c, --clean [y/N]      Reset all change to latest commit (default: n)
EOF
}
...
FUNC_CHECKENV()
{
    ...
    if [ ! -z $RELEASE ]; then
        ...
        export KBUILD_BUILD_HOST="OpenELA-LTS"
        LOCAL=y
    fi
}
...
CLANG=$RDIR/toolchain/clang-r450784/bin/
ARGS="
    ARCH=arm64 O=out \
    CC=${CLANG}clang \
...
if [[ "$KSU" == "y" ]]; then
    ...
    if test -d "$DIR/drivers/kernelsu" && grep -rnw 'fs/Makefile' -e 'CONFIG_KSU_SUSFS'; then
        ...
        if ! grep -rnw 'fs/Makefile' -e 'CONFIG_KSU_SUSFS'; then
            ...
            curl -LOSs "https://raw.githubusercontent.com/oItsMineZKernel/Kernel-Patch/main/SuSFS.patch" && patch -p1 < SuSFS.patch && rm -rf *.patch
        fi
    fi
fi
...
FUNC_TOOLCHAIN()
{
    ...
    git submodule add --force https://gitlab.com/crdroidandroid/android_prebuilts_clang_host_linux-x86_clang-r450784.git toolchain/clang-r450784
}
...
FUNC_BUILD_KERNEL()
{
    ...
    sed -i "s/CONFIG_LOCALVERSION=\"\"/CONFIG_LOCALVERSION=\"-oItsMineZKernel-$KERNEL_VERSION-$DEVICE-$MODEL\"/" $RDIR/arch/arm64/configs/$KERNEL_DEFCONFIG
    sed -i "s/CONFIG_LOCALVERSION_AUTO=\"y\"/CONFIG_LOCALVERSION_AUTO=\"n\"/" $RDIR/arch/arm64/configs/$KERNEL_DEFCONFIG
...
{
    # Build dtb
    echo "-- Building exynos$SOC Device Tree Blob Image..."
    echo "---------------------------------------------------------"
    ...
    # Build dtbo
    echo "---------------------------------------------------------"
    echo "-- Building Device Tree Blob Image for "$MODEL"..."
    echo "---------------------------------------------------------"
...
FUNC_BUILD_RAMDISK()
{
    ...
    cd $RDIR/build/AIK && ./repackimg.sh --nosudo
}
...
FUNC_BUILD_ZIP()
{
    ...
    echo -e "#MAGISK" > $RDIR/build/out/$MODEL/zip/module/META-INF/com/google/android/updater-script
    ...
    sed -i "s/CONFIG_LOCALVERSION=\"-oItsMineZKernel-$KERNEL_VERSION-"$DEVICE"-$MODEL\"/CONFIG_LOCALVERSION=\"-oItsMineZKernel-$KERNEL_VERSION-"$DATE"-"$DEVICE"-$MODEL\"/" $RDIR/arch/arm64/configs/$KERNEL_DEFCONFIG

    NAME=$(grep -o 'CONFIG_LOCALVERSION="[^"]*"' $RDIR/arch/arm64/configs/$KERNEL_DEFCONFIG | cut -d '"' -f 2)
    NAME=${NAME:1}.zip
...
FUNC_CLEANUP()
{
    ...
    sed -i "s/CONFIG_LOCALVERSION=\"-oItsMineZKernel-$KERNEL_VERSION-$DATE-$DEVICE-$MODEL\"/CONFIG_LOCALVERSION=\"\"/" $RDIR/arch/arm64/configs/$KERNEL_DEFCONFIG
    sed -i "s/CONFIG_LOCALVERSION_AUTO=\"n\"/CONFIG_LOCALVERSION_AUTO=\"y\"/" $RDIR/arch/arm64/configs/$KERNEL_DEFCONFIG
